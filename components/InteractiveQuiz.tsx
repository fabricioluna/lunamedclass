...
const TopProgressBar = ({ progress, currentIndex, totalQuestions }: { progress: number, currentIndex: number, totalQuestions: number }) => (
  <div className="sticky top-0 z-40 bg-[#f4f7f6] pt-4 pb-4 border-b border-gray-200/50 shadow-sm -mx-4 px-4 sm:-mx-6 sm:px-6 mb-6">
    <div className="flex justify-between items-end mb-2 px-2 max-w-4xl mx-auto">
      ...
      <div className="text-right">
        ...
        <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">Questão {currentIndex + 1} de {totalQuestions}</p>
      </div>
    </div>
    <div className="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden max-w-4xl mx-auto">
      <div className="bg-[#003366] h-full transition-all duration-700 ease-in-out" style={{ width: `${progress}%` }}></div>
    </div>
  </div>
);
  ...
  <div className="fixed bottom-6 left-0 right-0 z-50 px-4 pointer-events-none">
    <div className="max-w-md mx-auto bg-[#003366] text-white p-3 rounded-3xl shadow-xl flex items-center justify-between border-2 border-[#D4A017] pointer-events-auto">
      <div className="flex items-center gap-3">
        <button onClick={onOpenGrid} className="bg-white/10 p-3 rounded-xl hover:bg-white/20 transition-all flex items-center gap-2" title="Abrir mapa de questões">
        ...
        <div className="hidden sm:block">
          ...
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-[10px] font-bold opacity-40">/ {totalQuestions} resp.</span>
          </div>
        </div>
      </div>
      <div className="flex items-center pl-2 border-l border-white/10">
        <div className="pr-3 text-right">
          ...
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-[10px] font-bold opacity-40">/ {totalQuestions}</span>
          </div>
        </div>
      </div>
    </div>
...
const NavigationGridDrawer = ({ onClose, questions, answers, currentIndex, onJump }: any) => (
  <div className="fixed inset-0 z-[60] flex flex-col justify-end pointer-events-auto">
    ...
    <div className="bg-white rounded-t-[2rem] p-6 pb-24 shadow-2xl relative z-10 animate-in slide-in-from-bottom-8 duration-300 max-h-[80vh] overflow-y-auto">
      ...
      <div className="flex items-center gap-4 mb-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 gap-3 max-w-4xl mx-auto">
        {questions.map((quest: Question, idx: number) => {
          ...
          if (idx === currentIndex) btnClass += "ring-4 ring-[#D4A017] ring-offset-2 ";

          if (!isQAnswered) btnClass += "bg-gray-100 text-gray-400 hover:bg-gray-200";
          else if (isQCorrect) btnClass += "bg-green-500 text-white shadow-sm";
          else btnClass += "bg-red-500 text-white shadow-sm";

          return (
            <button key={idx} onClick={() => onJump(idx)} className={btnClass}>
              {idx + 1}
            </button>
          );
        })}
...
const InteractiveQuiz: React.FC<InteractiveQuizProps> = ({ questions, onFinish, onAnswerQuestion, storageKey, resumeState }) => {
  ...
  // FIX: Contagem exata de respondidas apenas para as questões atuais
  const answeredCount = Object.keys(answers).filter(id => questions.some(q => q.id === id)).length;
  const unansweredCount = questions.length - answeredCount;

  useEffect(() => {
    // FIX BUG-UI-004: Adicionamos a "Assinatura" do simulado no cache.
    const quizSignature = questions.map(q => q.id).join(',');
    const stateToSave = { currentIndex, answers, score, draftAnswers, eliminatedOptions, quizSignature };
    localStorage.setItem(storageKey, JSON.stringify(stateToSave));
  }, [currentIndex, answers, score, draftAnswers, eliminatedOptions, storageKey, questions]);
  ...
  const toggleEliminateOption = (questionId: string, optionIndex: number) => {
    ...
    setEliminatedOptions((prev: Record<string, number[]>) => {
      ...
      if (isEliminated) newEliminated = currentEliminated.filter(idx => idx !== optionIndex);
      else newEliminated = [...currentEliminated, optionIndex];
      ...
      <div className="animate-in fade-in slide-in-from-right-4 duration-500">
        <div 
          className={`bg-white rounded-[2rem] shadow-lg transition-all duration-700 overflow-hidden border-2
            ${isAnswered ? (isCorrect ? 'border-green-500 shadow-green-100' : 'border-red-400 shadow-red-100') : 'border-transparent'}
          `}
        >
          {q.image && (
            <div className="w-full h-48 md:h-64 bg-gray-100 border-b overflow-hidden group">
              <img src={q.image} alt="Material Prático" className="w-full h-full object-contain transition-transform duration-500 group-hover:scale-105" />
            </div>
          )}
          ...
          <div className="p-5 md:p-8">
              ...
              {q.options.map((opt, optIdx) => {
                ...
                return (
                  <div key={optIdx} className={wrapperClass}>
                    <button disabled={isAnswered || isEliminated} onClick={() => handleSelectOption(q.id, optIdx)} className="flex-1 text-left p-3 md:p-4 flex items-center gap-3 outline-none focus:outline-none">
                      <span className={letterClass}>{String.fromCharCode(65 + optIdx)}</span>
                      <span className={textClass}>{opt}</span>
                    </button>
                    {!isAnswered && (
                      <div className="flex items-center pr-3">
                        <button onClick={() => toggleEliminateOption(q.id, optIdx)} className={`p-2 rounded-lg transition-all ${isEliminated ? 'bg-red-100 text-red-500 hover:bg-red-200' : 'bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-700'}`} title={isEliminated ? "Restaurar alternativa" : "Eliminar alternativa"}>
                          <Scissors size={16} />
                        </button>
                      </div>
                    )}
            ...
            {!isAnswered && draftAnswers[q.id] !== undefined && (
              <div className="mt-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <button onClick={() => confirmAnswer(q.id, q.answer)} className="w-full bg-[#003366] text-white py-3.5 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-[#D4A017] hover:text-[#003366] transition-all shadow-md">
                  Confirmar Resposta
                </button>
              </div>
            )}
            ...
            {isAnswered && (
              <div className={`mt-6 p-5 md:p-6 rounded-2xl animate-in fade-in slide-in-from-top-4 duration-500 border ${isCorrect ? 'bg-green-50 border-green-200 text-green-900' : 'bg-red-50 border-red-200 text-red-900'}`}>
                <div className="flex items-center gap-3 mb-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shadow-sm ${isCorrect ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
                    {isCorrect ? '✓' : '✕'}
                  </div>
                  <div>
        ...
        <div className="mt-6 flex justify-between items-center gap-3">
          <button onClick={prevQuestion} disabled={currentIndex === 0} className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all ${currentIndex === 0 ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : 'bg-white border-2 border-[#003366] text-[#003366] hover:bg-gray-50 shadow-sm'}`}>
            <ArrowLeft size={14} /> Anterior
          </button>
          
          {!isLastQuestion ? (
            <button onClick={nextQuestion} className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all ${isAnswered ? 'bg-[#003366] text-white hover:bg-[#D4A017] hover:text-[#003366] shadow-md' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>
              {isAnswered ? 'Próxima' : 'Pular' } {isAnswered ? <ArrowRight size={14} /> : <SkipForward size={14} />}
            </button>
          ) : (
            <button onClick={() => onFinish(score, answers)} className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all bg-[#D4A017] text-[#003366] hover:scale-105 shadow-md">
              Ver Relatório
            </button>
          )}
        </div>
