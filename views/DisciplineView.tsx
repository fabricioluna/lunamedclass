...
import { Lock, Stethoscope, BookOpen, FolderOpen, PenTool, ClipboardSignature, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-32">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        ...
        {/* HUB UNIFICADO: SIMULADOS DE PRÁTICA (Onde a mágica acontece) */}
        <button 
          onClick={() => handleAction('lab_osce', isUC ? 'lab-list' : 'osce-mode-selection')} 
          className={`p-6 md:p-8 rounded-[2rem] text-left transition-all group border-2 relative overflow-hidden ${
            isPracticalLocked 
              ? 'bg-gray-50 border-gray-200 opacity-70 grayscale cursor-not-allowed' 
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl transition-all ${
              isPracticalLocked ? 'bg-gray-200 text-gray-400' : 'bg-white/10 group-hover:bg-[#D4A017] group-hover:text-[#003366]'
            }`}>
               <Stethoscope size={28} />
            </div>
            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center font-black ${
              isPracticalLocked ? 'border-gray-300 text-gray-300' : 'border-white/20 group-hover:border-[#D4A017] group-hover:text-[#D4A017]'
            }`}>→</div>
          </div>
          ...
          <p className={`text-xs font-medium leading-relaxed relative z-10 ${isPracticalLocked ? 'text-gray-400' : 'opacity-90'}`}>
            {isUC 
