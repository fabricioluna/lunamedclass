...
const DisciplineView: React.FC<DisciplineViewProps> = ({ disciplineId, disciplines, summaries, onBack, onSelectOption }) => {
  ...
  // Consome a árvore de Feature Flags globais em tempo real via contexto de dados
  ...
  const isPraticaClinicaActive = checkFlag('modulo_pratica_clinica', true);
  ...
  const handleAction = (featureId: string, action: string) => {
    if (featureId === 'quiz' && !isSimuladoTeoricoActive) {
      alert("O módulo de Simulado Teórico está temporariamente desativado pela coordenação.");
      return;
    }
    if (featureId === 'materials' && !isMateriaisActive) {
  ...
  const handleLabClick = () => {
    if (!isPraticaClinicaActive) {
      alert("O módulo de práticas clínicas e simulações foi pausado globalmente.");
      return;
    }
    if (isUC && !isLabActive) {
